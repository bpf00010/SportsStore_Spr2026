    ...
    public class ProductRepository : IProductRepository
    {
        ...
        public bool EditProduct(Product product)
        {
...

            {
                ...
                {
                    command.CommandType = System.Data.CommandType.StoredProcedure;
                    command.Parameters.Add(new SqlParameter("@prodID", product.ProductId));
                    ...
                    command.Parameters.Add(new SqlParameter("@thumbnail", product.thumbnail));
                    command.Parameters.Add(new SqlParameter("@image", product.image));
                    command.Parameters.Add(new SqlParameter("@PromoFront", product.PromoFront));
                    command.Parameters.Add(new SqlParameter("@PromoDept", product.PromoDept));

                    //need a sql command that will execute the stored procedure that will update the product in the database with the new values from the product object that was passed in and then we will return true if the update was successful and false if it was not successful. We can use the execute non query method to execute the stored procedure and then we can check the number of rows affected to determine if the update was successful or not. If the number of rows affected is greater than 0 then we can return true, otherwise we can return false.
                    //so we can use the execute non query method to execute the stored procedure and then we can check the number of rows affected to determine if the update was successful or not. If the number of rows affected is greater than 0 then we can return true, otherwise we can return false.
                    return command.ExecuteNonQuery() > 0;
                    //gives back an intiger 
                ...
                //4. Run the command and get the data
                //5. Close the connection
                //this method will take a product object and update the corresponding product in the database with the new values from the product object




                //in this case we should return true or false, but what should we return once the stored procedure goes through? We should return.
